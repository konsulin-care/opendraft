...
describe('CommitDialog', () => {
  ...
  it('renders commit dialog with message input', () => {
    render(
      <CommitDialog isOpen={true} onClose={mockOnClose} onCommit={mockOnCommit} workspace={workspace} manifestPath="blocks/manifest.json" />
    );
  ...
  it('does not render when isOpen is false', () => {
    const { container } = render(
      <CommitDialog isOpen={false} onClose={mockOnClose} onCommit={mockOnCommit} workspace={workspace} manifestPath="blocks/manifest.json" />
    );
    expect(container.innerHTML).toBe('');
  });

  it('shows validation errors when commit fails', async () => {
